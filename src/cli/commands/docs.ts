  ...
  export: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "`--project` (requires `--out-dir`) generates an installable Playwright project instead of standalone spec files: `package.json` (`@playwright/test`, TypeScript, and test/typecheck scripts), strict `tsconfig.json` with DOM/Node types and portable `.ts` imports, `playwright.config.ts` (baseURL from config, serial `workers: 1`, `bypassCSP: true`, `globalSetup` wired), `global-setup.ts`, `actions/<name>.ts` (each reusable action from `imports:` becomes one exported `async function(page)` that tests import), `verifiers/` (node verifier files plus a bounded safe relative dependency closure), `tests/<spec>.spec.ts`, and an executable `README.md`. Run `npm install`, `npx playwright install chromium`, `npm run typecheck`, then `npm test`.",
      },
      {
      ...
      },
      {
        ...
        body: "When writing files, stdout is a structured report: status written|partial|error, per-file coverage (steps/outcomes exported vs total), and skips with reasons. Partial means at least one construct was commented/skipped (eval.file, an unreadable browser script.file, inline runtime: node scripts, transform, snapshot, monitor, unresolved use:). Agents should read `coverage.skips` in `--format json` before treating a handoff as complete.",
      },
      {
