  ...
  authoring: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "Set the browser viewport per environment (`environments.<env>.viewport: { width, height }`) or per spec (top-level `viewport:`); spec wins. Bound artifact disk usage with `retention`: `keepRuns` (newest N runs per spec, pruned after every run — default 3 when no `retention` block is set; `retention: { enabled: false }` keeps everything) and `archiveToStash: true` (archives pruned run dirs to fcheap before deletion — best-effort; if the archive fails the run is retained on disk so no artifacts are lost). `cairn clean [--keep N | --all]` prunes manually with the same defaults. Traces follow `artifacts.capture.trace` — the on-failure default deletes the trace zip on passing runs. Videos follow `artifacts.capture.video` (default `never`) — opt in with `always` or `on-failure` for audit-grade recordings.",
      },
      {
        title: "Logging",
